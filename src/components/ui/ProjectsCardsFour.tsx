...
interface ResultCardProps {
  ...
  title: string;
  content: string;
  isActive?: boolean; // Kept for type compatibility, but active state is now determined by position
}
...
// Updated to have 5 cards per project for smoother slider flow
const projectsData: {
  id: string;
        ...
        >
              ...
              <div key={item.id}>
                ...
                {item.type === "problem" && (
                  <motion.div
                    ...
                    className="bg-white/20 border border-white/80 shadow-lg w-[324px] h-[120px] rounded-xl p-4"
                  >
                ...
                {item.type === "industry" && (
                  <motion.div
                    ...
                    className="relative w-[270px] h-[100px] flex items-center gap-3 rounded-[10px] hover:bg-white/30 backdrop-blur-[50px]"
                  >
                    <Image
          ...
          >
            {resultsData.map((result, rIndex) => {
              ...
              // This creates a continuous rotation: 0 -> 4 -> 3 -> 2 -> 1 -> 0
              const slot = (rIndex - activeCycle + 5) % 5;

              // Define the "active" slot as the one in the middle (Slot 0 in our logic below)
              // Visual Layout Order on Y-Axis:
              // Slot 3: Top Hidden    (y: -260)
              // Slot 4: Top Visible   (y: -130)
              // Slot 0: Center Active (y: 0)
              // Slot 1: Bottom Visible(y: 130)
              // Slot 2: Bottom Hidden (y: 260)
              ...
              switch (slot) {
                ...
                case 2: // Bottom Hidden (Exit point)
                  ...
                  opacity = 0;
                  zIndex = 0;
                  // If we are here, we are about to jump to top.
                  // When a card enters this slot, it fades out.
                  break;
              ...
              // Special handling for the "teleport" from Bottom Hidden to Bottom (or Top Hidden to Bottom Hidden logic)
              // If we are in slot 2 (Bottom Hidden), we want normal animation to get there.
              // If we are in slot 3 (Top Hidden), we want normal animation to get there?
              // The logic: 0->4 (Move Up). 4->3 (Move Up). 3->2 (Teleport Jump). 2->1 (Move Up). 1->0 (Move Up).
              // So the jump happens when going from Slot 3 to Slot 2.
              // Since both 3 and 2 are Opacity 0, we can make the duration 0 to satisfy the loop.
              if (slot === 2) {
...
export default function ProjectsCardsFour() {
